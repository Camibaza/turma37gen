				...
				do {
					...
					for(Produto prod : vitrine) {
						if(codigoProduto.equals(prod.getCodigo())) {
							System.out.println("\nCONFIRMA O PRODUTO:\n" + prod.getCodigo() + "\t" + prod.getProduto() + "\t" + "R$" + prod.getValor() );
							...
							if(confirmaProduto.equals("S")) {
								...
								if(quantidadeCarrinho > prod.getEstoque()) {
									...
									if(carrinho.verificaCarrinho(prod.getCodigo(), quantidadeCarrinho)) {
										carrinho.mostraCarrinho();
//										carrinho.setValorTotal(carrinho.getQuantidade() * carrinho.getValor());
									} else {
										carrinho.entraCarrinho(prod.getProduto(), prod.getCodigo(), prod.getValor(), quantidadeCarrinho);
										prod.retiraEstoque(quantidadeCarrinho);
										carrinho.mostraCarrinho();
//										carrinho.setValorTotal(quantidadeCarrinho * carrinho.getValor());
									}	
								}	
							} 
